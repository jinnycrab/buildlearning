...
export const ProcessCard = ({
  ...
  if (isMobile) {
    return <div className="relative h-[calc(100vh-320px)] rounded-xl overflow-hidden group">
        {/* Image with overlay gradient */}
        <div className="absolute inset-0">
          <img src={image} alt={`${title} Process`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
  ...
  return <div className="relative h-[450px] rounded-xl overflow-hidden group">
      {/* Image with overlay gradient */}
      <div className="absolute inset-0">
        <img src={image} alt={`${title} Process`} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" />
        <div className="absolute inset-0 bg-gradient-to-b from-black/60 via-black/70 to-black/90" />
