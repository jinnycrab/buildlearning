...
const Navigation = () => {
  ...
  return <nav className="border-b bg-white sticky top-0 z-50">
      <div className="container flex items-center justify-between py-0">
        ...
        {/* Mobile Menu Button */}
        <button className="md:hidden p-2 mr-4" onClick={() => setIsMenuOpen(!isMenuOpen)}>
        ...
        {isMenuOpen && <div className="absolute top-full left-0 right-0 bg-white border-b md:hidden">
            <div className="flex flex-col p-4 space-y-4">
              <Link to="/about" className="text-muted-foreground hover:text-accent px-4 py-2" onClick={() => setIsMenuOpen(false)}>Process</Link>
              <Link to="/process" className="text-muted-foreground hover:text-accent px-4 py-2" onClick={() => setIsMenuOpen(false)}>
                Pedagogy
              </Link>
              <Link to="/resources" className="text-muted-foreground hover:text-accent px-4 py-2" onClick={() => setIsMenuOpen(false)}>Projects</Link>
